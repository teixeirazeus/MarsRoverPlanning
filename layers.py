...
class ActionNode(object):
    ...
    def __eq__(self, other):
        return isinstance(other, ActionNode) and self.expr == other.expr
...
class BaseActionLayer(BaseLayer):
    def __init__(
        self, actions=[], parent_layer=None, serialize=True, ignore_mutexes=False
    ):
    ...
    def update_mutexes(self):
        for actionA, actionB in combinations(iter(self), 2):
            ...
            elif self._inconsistent_effects(actionA, actionB) or self._interference(
                actionA, actionB
            ):
                self.set_mutex(actionA, actionB)
    ...
    def update_mutexes(self):
        for literalA, literalB in combinations(iter(self), 2):
            if self._negation(literalA, literalB):
                ...
                continue
            elif len(self.parent_layer) and self._inconsistent_support(
                literalA, literalB
            ):
                self.set_mutex(literalA, literalB)
